...
def parabricks_output():
    ...
    output.extend(expand("results/VCFs/merged_samples.vcf.gz"))
    output.extend(expand("results/reference/{ref_acc}.{ext}", ref_acc=config["ref_acc"], ext=["sa", "pac", "bwt", "ann", "amb"]))
    return output
